...
import { SaraBissoCtaNewsProps } from "@/lib/types/SaraBissoCtaNewsProps";
import Link from "next/link";

export default function SaraBissoCtaNews({
  saraBissoCtaNewsData,
}: SaraBissoCtaNewsProps) {
  const featured = saraBissoCtaNewsData.filter(
    (item: any) => item.is_featured === 1,
  );
  const nonFeatured = saraBissoCtaNewsData.filter(
    (item: any) => item.is_featured !== 1,
  );
