...
import { JuktorastroNewsProps } from "@/lib/types/JuktorastroNewsProps";
import Link from "next/link";
import React from "react";

export default function JuktorastroCtaNews({
  juktorastroNewsData,
}: JuktorastroNewsProps) {
  const featured = juktorastroNewsData.filter(
    (item: any) => item.is_featured === 1,
  );
  const nonFeatured = juktorastroNewsData.filter(
    (item: any) => item.is_featured !== 1,
  );
  ...
  return (
    <div>
      <SectionHeader title="যুক্তরাষ্ট্র" />
