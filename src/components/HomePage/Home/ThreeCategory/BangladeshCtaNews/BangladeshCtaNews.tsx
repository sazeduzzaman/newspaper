...
import { BangladeshNewsProps } from "@/lib/types/BangladeshNewsProps";
import Link from "next/link";

export default function BangladeshCtaNews({
  bangladeshNewsData,
}: BangladeshNewsProps) {
  const featured = bangladeshNewsData.filter(
    (item: any) => item.is_featured === 1,
  );
  const nonFeatured = bangladeshNewsData.filter(
    (item: any) => item.is_featured !== 1,
  );
